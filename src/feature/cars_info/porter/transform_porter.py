...
def transform_porter_data(raw_data_path: Path = None):
    """포터 차량 데이터 변환"""
    if raw_data_path is None:
        # json 저장 경로
        raw_data_path = Path(__file__).parents[4] / "data" / "raw" / "car_info" / "porter.json"
    
    # JSON 파일이 없으면 빈 리스트 반환
    if not raw_data_path.exists():
        print(f"[PORTER] 원본 데이터 파일이 없습니다: {raw_data_path}")
        return []
    ...
    for model_key, model_data in raw_data.items():
        ...
        # 모델명 또는 모델 키에 "EV" 또는 "일렉트릭"이 포함되어 있으면 전기로 설정
        is_ev_model = "EV" in model_key.upper() or "일렉트릭" in model_name or "EV" in model_name.upper()
        ...
        for lineup in model_data.get("lineup", []):
            ...
            # 연료 타입: 모델명에 EV/일렉트릭이 있으면 전기, 아니면 specs에서 추출
            if is_ev_model:
                fuel_type = "전기"
            else:
