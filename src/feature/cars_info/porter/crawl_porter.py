...
from config import PORTER_MODELS
...
def extract_model_image_url(driver, model_id, search_keyword="포터"):
    """
    다나와 검색 페이지에서 특정 model_id의 대표 이미지 URL을 추출
    """
    search_keywords = [search_keyword]
    if "일렉트릭" in search_keyword or "EV" in search_keyword:
        search_keywords = [search_keyword, "포터2 일렉트릭", "포터"]
    elif "포터2" in search_keyword:
        search_keywords = [search_keyword, "포터"]
...
def crawl_model(driver, model_id, model_name, lineup_ids=None):
    ...
    try:
        ...
        search_keyword = "포터2 일렉트릭" if "일렉트릭" in model_name or "EV" in model_name else "포터2"
    ...
    try:
        if lineup_ids:
            ...
            for lineup_id in lineup_ids:
                lineup_id_str = str(lineup_id)
                lineup_name = lineup_map.get(lineup_id_str, f"라인업 {lineup_id}")
                lineups.append({"id": lineup_id_str, "name": lineup_name})
...
def main():
    ...
    명령줄 인자:
        ...
        예: python crawl_porter.py --lineup-ids 53592,53588
    """
    # 명령줄 인자 파싱
    lineup_ids_arg = None
    if len(sys.argv) > 1:
    ...
    logger.info(f"대상 모델: {len(PORTER_MODELS)}개")
    logger.info("=" * 50)
    ...
    try:
        for idx, (key, model_info) in enumerate(PORTER_MODELS.items(), 1):
            logger.info(f"\n[{idx}/{len(PORTER_MODELS)}] 모델 처리 중...")
